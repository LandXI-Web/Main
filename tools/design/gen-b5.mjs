...
const TW = 240, TH = 147, COLS = [128, 380, 632, 884, 1136], ROWS = [140, 326, 512, 698]   /* +2 스케일: 캡션 2줄(16+15) 이 다음 행에 닿지 않게 */;
...
const shelf = (state, actions) => `<div style="position:absolute;left:0;right:0;bottom:0;height:24px;background:${INK};display:flex;align-items:center;padding:0 8px;gap:8px">
<div class="n" style="font-size:14px;letter-spacing:0;color:#FFFFFF;white-space:nowrap;flex:none">${state}</div>
<div class="n" style="font-size:14px;letter-spacing:0;color:rgba(255,255,255,.72);margin-left:auto;white-space:nowrap;min-width:0;overflow:hidden;text-overflow:ellipsis">${actions.join(' · ')}</div></div>`;
const word = (t, dark) => `<div class="n" style="position:absolute;left:8px;top:8px;font-size:14px;letter-spacing:.08em;color:${dark ? '#FFFFFF' : G};white-space:nowrap">${t}</div>`;
const stepLabel = (t) => `<div class="n" style="position:absolute;left:8px;bottom:7px;font-size:14px;letter-spacing:0;color:#FFFFFF;white-space:nowrap;flex:none">${t}</div>`;
const demo = (dark) => `<span class="n" style="border:1px dotted ${dark ? 'rgba(255,255,255,.5)' : C};color:${dark ? 'rgba(255,255,255,.7)' : C};padding:0 3px;font-size:14px;letter-spacing:.06em">시연</span>`;

function cap(name, meta, opts = {}) {
  const c = opts.dim ? C : INK, m = opts.dim ? C : G;
  let s = `<div class="n" style="margin-top:7px;font-size:14px;line-height:16px;letter-spacing:-.005em;display:flex;gap:0;color:${c}"><span style="flex:0 1 auto;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${name}</span><span style="flex:none;white-space:nowrap;color:${m}">&nbsp;· ${meta}</span></div>`;
  if (opts.sub) s += `<div class="n" style="font-size:14px;line-height:15px;margin-top:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:${INK}"><span style="color:${AMB}">■</span> ${opts.sub}</div>`;
  return s;
}
...
const tree = (color) => `<div class="n" style="position:absolute;left:12px;top:26px;font-size:14px;line-height:19.5px;letter-spacing:.01em;color:${color};white-space:pre">camera_org_202604/
├ 20260412/
│  ├ DJI_0001.JPG
│  ├ DJI_0002.JPG
│  └ … 4,820장
...
<div style="font-size:15px;letter-spacing:-.014em;color:${INK}">끌어다 놓거나 클릭</div>
<div class="n" style="font-size:14px;letter-spacing:.03em;color:${G}">최대 1 TB · ECW TIF ZIP SHP XLSX</div></div>`,
...
<table class="n" style="position:absolute;left:10px;top:30px;width:222px;border-collapse:collapse;font-size:14px;line-height:20px;letter-spacing:0;white-space:nowrap;color:${INK}">
<tr style="color:${G};border-bottom:1px solid ${INK}"><td>pnu</td><td>emd</td><td>cls</td><td style="text-align:right">area</td></tr>
<tr style="border-bottom:1px solid ${H}"><td>…0001</td><td>사매면</td><td>경작</td><td style="text-align:right">1,284</td></tr>
<tr style="border-bottom:1px solid ${H}"><td>…0007</td><td>사매면</td><td>비경작</td><td style="text-align:right">612</td></tr>
<tr style="border-bottom:1px solid ${H}"><td>…0012</td><td>사매면</td><td>경작</td><td style="text-align:right">2,031</td></tr>
...
<div class="n" style="position:absolute;right:8px;bottom:7px;font-size:14px;letter-spacing:.06em;color:${G}">EPSG:5186 · 456 polygon</div>`,
...
<div class="n" style="position:absolute;right:8px;top:23px;font-size:14px;letter-spacing:.06em;color:${G}">EPSG 없음 · 53 polygon</div>${shelf('실패 1/4 파일 확인', ['좌표계 지정', '발행 취소', '세부 정보'])}`,
...
<div style="font-size:14px;letter-spacing:-.01em;color:${G}">미리보기 없음</div>
<div class="n" style="font-size:14px;letter-spacing:.04em;color:${C}">좌표계 없음 · 판에 세우지 않는다</div></div>${word('아카이브 · 공간정보 · 표시', false)}`,
...
const chip = (t, n, on) => `<div style="display:flex;align-items:baseline;gap:6px;border:1px solid ${on ? ACC : INK};${on ? `background:${TINT};` : ''}padding:6px 10px 6px;font-size:14px;letter-spacing:-.014em;color:${on ? ACC : INK};white-space:nowrap">${t}<span class="n" style="font-size:14px;color:${G}">${n}</span></div>`;
const dd = (t) => `<div style="display:flex;align-items:center;gap:8px;border:1px solid ${H};padding:6px 10px;font-size:14px;letter-spacing:-.014em;color:${G};white-space:nowrap">${t}<svg width="9" height="6" viewBox="0 0 9 6" fill="none" stroke="${G}" stroke-width="1.25"><path d="M.5.5 4.5 5 8.5.5"/></svg></div>`;

const head = `
<div style="position:absolute;left:128px;top:19px;display:flex;align-items:baseline;gap:14px">
<div class="d" style="font-size:23px;line-height:28px;letter-spacing:-.02em">데이터 관리</div>
<div class="n" style="font-size:14px;letter-spacing:.02em;color:${G}">25건 · 업로드 6 · 완료 8 · 발행중 7 · 아카이브 4</div></div>
<div style="position:absolute;right:56px;top:0;height:64px;display:flex;align-items:center;gap:14px">
<div style="display:flex;align-items:baseline;gap:3px"><div class="n" style="font-size:66px;line-height:66px;letter-spacing:-.03em;color:${INK}">96</div><div class="n" style="font-size:20px;color:${G}">%</div></div>
<div style="display:flex;flex-direction:column;gap:2px;line-height:1.1">
<div class="n" style="font-size:14px;letter-spacing:.1em;color:${G}">내 디스크 사용량 · ${demo(false)}</div>
<div class="n" style="font-size:14px;letter-spacing:.02em;color:${INK}">1,965.0 / 2,048.0 GB · 잔여 83.0 GB</div>
<div style="font-size:14px;letter-spacing:-.01em;color:${INK};border-bottom:1px solid ${INK};align-self:flex-start">디스크 증량 신청 ›</div></div></div>
...
<div style="font-size:14px;color:${C};letter-spacing:-.01em;white-space:nowrap">파일명 · 데이터명</div></div>
...
<div style="font-size:14px;color:${G}">개인정보처리방침</div><div style="font-size:14px;color:${G}">이용약관</div><div style="font-size:14px;color:${G}">이메일주소무단수집거부</div>
<div class="n" style="font-size:14px;letter-spacing:.02em;color:${C}">(우)54870 전북 전주시 덕진구 기지로 120 · 고객센터 063-713-1213, 1216</div>
<div style="font-size:14px;color:${G};display:flex;align-items:center;gap:6px">Family Site<svg width="8" height="5" viewBox="0 0 9 6" fill="none" stroke="${G}" stroke-width="1.25"><path d="M.5.5 4.5 5 8.5.5"/></svg></div></div>
`;
...
<div class="n" style="width:78px;flex:none;font-size:14px;letter-spacing:.06em;color:${G}">${label}${req ? ` <span style="color:${INK}">*</span>` : ''}</div>
<div style="flex:1;font-size:14.5px;letter-spacing:-.01em;color:${INK};display:flex;align-items:center;justify-content:space-between">${value}${dropdown ? `<svg width="9" height="6" viewBox="0 0 9 6" fill="none" stroke="${INK}" stroke-width="1.25"><path d="M.5.5 4.5 5 8.5.5"/></svg>` : ''}</div></div>`;
...
const permRow = (org, sel) => `<div style="display:flex;align-items:baseline;padding:10px 0 9px;border-bottom:1px solid ${H}"><div style="flex:1;font-size:14px;letter-spacing:-.01em;color:${INK}">${org}</div><div class="n" style="font-size:14px;letter-spacing:.02em">${seg(sel)}</div></div>`;
...
<div class="n" style="font-size:14px;letter-spacing:.1em;color:${G}">업로드 완료 → 지도 레이어 발행</div>
<div class="d" style="font-size:23px;line-height:28px;letter-spacing:-.02em;margin-top:6px">지도 레이어 발행</div>
<div class="n" style="font-size:14px;letter-spacing:.01em;color:${G};margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">NW_ortho_202604_zone_X.ecw · 47.6 GB · 2026.04.12 · 아카이빙 1회 ${demo(false)}</div>
...
<div class="n" style="position:absolute;left:10px;bottom:8px;font-size:14px;letter-spacing:.05em;color:#FFFFFF">127.3481,35.5276 ~ 127.3567,35.5347 · EPSG:5186 → 4326 · GSD 1.69 cm</div>
<div class="n" style="position:absolute;right:10px;top:8px;font-size:14px;letter-spacing:.06em;color:rgba(255,255,255,.7);border:1px solid rgba(255,255,255,.5);padding:0 3px">측정</div></div>
...
<div class="n" style="font-size:14px;letter-spacing:.1em;color:${G}">공유 권한</div><div class="n" style="font-size:14px;letter-spacing:.06em;color:${G}">기관명 / 권한명</div></div>
${permRow('LX 한국국토정보공사', '편집')}
${permRow('남원시청', '뷰어')}
<div style="padding:8px 0 0;font-size:14px;color:${G}">+ 기관 추가</div>
<div style="display:flex;align-items:center;gap:10px;margin-top:26px">
<div style="position:relative;width:120px;height:38px;display:flex;align-items:center;justify-content:center">${br(120, 38, INK, 12, 1)}<span class="d" style="font-size:16px;letter-spacing:.01em">취소</span></div>
<div style="width:120px;height:38px;background:${INK};display:flex;align-items:center;justify-content:center"><span class="d" style="font-size:16px;color:#FFFFFF">발행</span></div>
<div class="n" style="font-size:14px;letter-spacing:.03em;color:${G};margin-left:6px;line-height:1.5">기준 일자 · 데이터명 필수</div></div>
</div>`;
