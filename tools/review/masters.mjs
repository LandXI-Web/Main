...
const STATUS = {
  ...
  'B5-Projects': ['프로젝트', 'review', '목록 · 구현 대기'],
  'B5-Project-Create': ['프로젝트', 'review', '만들기 5단계 워크플로우'],
...
var lb=document.getElementById('lb'),im=document.getElementById('lb-img'),t=document.getElementById('lb-t');function close(){lb.hidden=true;im.src='';document.body.style.overflow='';}
document.querySelectorAll('a.lb').forEach(function(a){a.addEventListener('click',function(e){e.preventDefault();im.src=a.getAttribute('href');t.textContent=a.dataset.title||'';lb.hidden=false;document.body.style.overflow='hidden';});});
document.getElementById('lb-x').addEventListener('click',close);lb.addEventListener('click',function(e){if(e.target===lb)close();});document.addEventListener('keydown',function(e){if(e.key==='Escape')close();});
})();</script></body></html>`;
