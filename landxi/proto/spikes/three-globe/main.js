...
const TARGET = { lon: 127.39, lat: 35.41, name: '남원 금지면' };
...
const sunDir = ll2v(12, 158).normalize();   // 태양 방향 — 한국이 아침 터미네이터에 걸리게
...
const EARTH_FS = `
  ...
  void main(){
    ...
    col += vec3(1.0,0.97,0.9) * pow(max(dot(n,H),0.0),140.0) * ocean * day * 1.35;
...
// 대기 (backside fresnel)
const ATMO_FS = `
  varying vec3 vP; varying vec3 vW; uniform vec3 uSun;
  void main(){
    vec3 n = normalize(vP);
    vec3 V = normalize(cameraPosition - vW);
    float f = pow(clamp(1.0-abs(dot(n,V)),0.0,1.0), 2.0);
    float sun = clamp(dot(n,uSun)*0.5+0.5, 0.0, 1.0);
    vec3 c = mix(vec3(0.10,0.24,0.62), vec3(0.55,0.72,1.0), sun);
    c += vec3(1.0,0.55,0.25)*pow(sun,6.0)*0.5;
    gl_FragColor = vec4(c, f*f*(0.16+0.9*sun));
  }`;
...
lScene.fog = new THREE.FogExp2(0x9fb6cf, 0.000021);

const lSunDir = new THREE.Vector3(0.42, 0.58, 0.70).normalize();
const lSun = new THREE.DirectionalLight(0xfff2dc, 3.4);
...
lScene.add(lSun);
lScene.add(new THREE.HemisphereLight(0xbcd6f2, 0x40403a, 1.1));
...
const sky = new THREE.Mesh(new THREE.SphereGeometry(240000, 48, 32), new THREE.ShaderMaterial({
  ...
  uniforms: {
    uHi: { value: new THREE.Color(0x0a1e46) }, uLo: { value: new THREE.Color(0xc3d6e8) },
    uK: { value: 0 }, uOp: { value: 0 },
  },
  ...
  fragmentShader: `varying vec3 vP; uniform vec3 uHi,uLo; uniform float uK,uOp;
    void main(){ float h=clamp(normalize(vP).y*0.5+0.5,0.0,1.0);
      gl_FragColor = vec4(mix(uLo,uHi,pow(h,0.75-0.35*uK)), uOp); }`,
}));
sky.renderOrder = -1;
lScene.add(sky);
...
groundPlane('ortho_z15.jpg', 'ortho_z15-bounds.json', 0, 1);      // 근접 16 km

/* 구름 빌보드 덱 — 하강하며 갈라진다 */
const decks = [];
{
  const files = ['cloud_far.webp', 'cloud_mid.webp', 'cloud_near.webp'];
  const alt = [7200, 3400, 1500], scale = [46000, 26000, 13000], op = [0.7, 0.82, 0.9];
  files.forEach((f, i) => {
    const t = texLoader.load('../../../assets/proto/clouds/' + f);
    t.colorSpace = THREE.SRGBColorSpace;
    for (let k = 0; k < 3; k++) {
      const m = new THREE.MeshBasicMaterial({ map: t, transparent: true, opacity: 0, depthWrite: false, fog: false });
      const q = new THREE.Mesh(new THREE.PlaneGeometry(scale[i], scale[i] * 0.62), m);
      q.rotation.x = -Math.PI / 2; q.rotation.z = Math.random() * Math.PI;
      const a = (k / 3) * Math.PI * 2 + i;
      q.position.set(Math.cos(a) * scale[i] * 0.30, alt[i], Math.sin(a) * scale[i] * 0.30 - 2000);
      q.renderOrder = 20 - i;
      q.userData = { base: q.position.clone(), i, op: op[i] * (k === 0 ? 1 : 0.72) };
...
const detectMat = new THREE.MeshBasicMaterial({
  color: 0x67e8ff, transparent: true, opacity: 0,
...
Promise.all([json('data/buildings.json'), json('data/greenhouse.json'), json('data/farmland.json')])
  .then(([bl, gh, fl]) => {
    ...
    (gh || []).forEach(f => push(f.r, 4.0));                    // 비닐하우스 4 m
    if (geos.length) {
      const mat = new THREE.MeshStandardMaterial({
        color: 0xdcd7cb, roughness: 0.7, metalness: 0.05, transparent: true, opacity: 0,
      });
    ...
    const quads = [];
    (fl || []).concat(gh || []).forEach(f => {
      const s = clamp(Math.sqrt(Math.max(f.a || 300, 150)) * 1.15, 14, 90);
      const g = new THREE.PlaneGeometry(s, s);
      g.rotateX(-Math.PI / 2);
      g.translate(m2x(f.c[0]), 4.8, m2z(f.c[1]));
      quads.push(g);
    });
    if (quads.length) {
...
const bloom = new UnrealBloomPass(new THREE.Vector2(innerWidth, innerHeight), 0.55, 0.7, 0.62);
composer.addPass(bloom);
const grade = new ShaderPass({
  uniforms: { tDiffuse: { value: null }, uT: { value: 0 }, uGrain: { value: 0.055 }, uVig: { value: 1.0 }, uCA: { value: 0.9 } },
  vertexShader: `varying vec2 vUv; void main(){ vUv=uv; gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0); }`,
  fragmentShader: `varying vec2 vUv; uniform sampler2D tDiffuse; uniform float uT,uGrain,uVig,uCA;
...
const GKEYS = [
  ...
  [0.14, 13500, 16, 78, 0],
  [0.30, 7200, 26, 106, 6],
  [0.46, 2600, 33.5, 121.5, 16],
  [0.58, 780, 35.1, 126.2, 30],
  [0.68, 190, 35.32, 127.05, 44],
  [0.76, 52, 35.39, 127.32, 56],
  [0.84, 16, 35.404, 127.375, 66],
];
// local: [p, altM, offsetX, offsetZ, pitchDeg, lookZ]
const LKEYS = [
  [0.62, 42000, -3600, 14000, 66, 0],
  [0.70, 17000, -2200, 7600, 60, 0],
  [0.80, 5200, -1100, 3400, 48, 0],
  [0.90, 1250, -380, 1450, 30, -300],
  [1.00, 175, 40, 420, 11, -1400],
];
...
function placeGlobeCam(p) {
  ...
  gCam.position.copy(tmpA).addScaledVector(north, Math.tan(tilt * D2R) * (alt / R_EARTH) * 1.9);
  gCam.up.copy(up);
  ll2v(lat, lon, 1, tmpB);
...
function placeLocalCam(p) {
  ...
  lCam.up.set(0, 1, 0);
  lCam.lookAt(ox * 0.22, 0, oz - d + lookZ);
...
function apply(p) {
  ...
  earthUniforms.uKoreaBlend.value = smooth(0.30, 0.56, p);

  // 크로스페이드 — 지구 표면 텍스처 → 우리 정사영상 평면 (고도 ≈ 40 km → 5 km)
  const handoff = smooth(0.655, 0.80, p);
  for (const m of fadeMats) m.opacity = handoff;
  detectMat.opacity = smooth(0.87, 0.98, p) * 0.5;
  ...
  lScene.fog.density = lerp(0.000030, 0.0000110, smooth(0.70, 1, p));

  decks.forEach(q => {
    const b = q.userData.base, i = q.userData.i;
    const near = smooth(0.62 + i * 0.05, 0.86 + i * 0.045, p);
    q.position.set(b.x * (1 + near * 2.4), b.y * (1 - near * 0.5) + near * 260, b.z * (1 + near * 2.2) + near * 7000);
    q.material.opacity = q.userData.op * smooth(0.58, 0.70, p) * (1 - smooth(0.84 + i * 0.04, 0.96, p)) * handoff;
  });
  ...
  bloom.strength = lerp(0.62, 0.28, smooth(0.6, 0.9, p));
  grade.uniforms.uGrain.value = lerp(0.062, 0.028, smooth(0.55, 0.95, p));
...
const CAPS = [
  ...
  [0.46, '대기권을 지난다', '움직이는 구름층 — NASA 구름 맵이 돌고, 실촬영 구름 덱이 갈라진다.'],
  [0.66, '남원, 금지면', '고도 40 km 아래에서 구면 텍스처를 우리 정사영상으로 넘긴다.'],
  [0.84, '마을이 서 있다', '건물·비닐하우스 발자국을 압출해 실제 그림자를 받는다.'],
