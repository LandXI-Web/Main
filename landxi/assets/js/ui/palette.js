...
export function initPalette({ sources, onPlace }) {
  ...
  function render() { const q = input.value.trim().toLowerCase(); items = sources().filter(i => !q || (i.label + ' ' + (i.sub || '')).toLowerCase().includes(q)).slice(0, 8); cur = 0;
    list.innerHTML = items.map((i, k) => `<li class="palette__item" role="option" data-type="${i.type}" aria-selected="${k === cur}"><span class="palette__type">${esc(TYPE[i.type])}</span><b>${esc(i.label)}</b><small>${esc(i.sub || '')}</small></li>`).join('') || `<li class="palette__empty">결과가 없습니다. 다른 이름이나 주소로 검색해 보세요.</li>`; }
