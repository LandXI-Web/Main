...
export const PROFILES = [
  ...
  { id: 'yeosu', region: '전라남도 여수시', code: '46130', scope: 'local',
    crs: 'EPSG:5186', area: 510, units: 25, unitLabel: '읍·면·동',
 ...
 *   원본 → 못 준다(위치만 알린다) · 타일 → 권한으로 닿게 한다 · 결과/라벨/모델 → 준다.
 * **이식의 실체가 여기 있다**: 새 지자체에 영상은 못 주지만 **학습데이터와 모델은 줄 수 있다**.
 * 그 지자체가 자기 영상만 올리면 같은 서비스가 선다.
 */
export const ASSET_TIERS = [
  { id: 'raw', name: '원본 정사영상', size: 'TB', share: 'none', keep: '기관 보관(LX · 지자체)',
    how: '복사·전송하지 않는다. 목록에 위치·촬영일·GSD만 등록한다.',
    why: '남원 1시점 2.1 TB — 회선·저장 비용이 사업비를 넘는다.' },
  { id: 'tile', name: '서비스 타일', size: 'GB', share: 'grant', keep: '플랫폼 타일 서버',
    how: '권한을 받은 기관이 지도에서 본다(다운로드 아님). 원본에서 1회 생성.',
    why: '행정 활용에 필요한 것은 원본이 아니라 화면에서 보이는 것이다.' },
 ...
 * R7 원본 정사영상은 복사해 보내지 않는다. 이식에 따라가는 것은 라벨·모델이고,
 *    영상은 현지에서 확보한다(ASSET_TIERS · transplantAssets).
 * ═══════════════════════════════════════════════════════════════════ */
