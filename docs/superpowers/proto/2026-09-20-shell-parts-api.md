...
## 함정 — 화면 CSS 가 셸에 지는 자리 (2026-09-20 두 그룹이 같은 데 걸렸다)

`shell.css` 의 이 한 줄이 페이지 클래스보다 특이도가 높다.

```css
.lx button{ font:inherit; color:inherit; background:none; border:0; padding:0; ... }
```

`.lx button` 은 (0,1,1) 이라 `.an button` `.pj-x` 같은 단일 페이지 클래스 (0,1,0) 을 **조용히 이긴다.**
버튼에 padding·배경·테두리를 주려면 반드시 `.lx` 를 앞에 붙인다.

```css
.pj-tool{ padding:0 12px; }        /* ✗ 먹지 않는다 */
.lx .pj-tool{ padding:0 12px; }    /* ✓ */
```

같은 이유로 페이지 클래스가 `<body class="lx pd">` 처럼 **body 에 함께 붙는 경우**
`.lx .pd ...` 는 후손 선택자라 맞지 않는다. `.lx.pd ...` 로 붙여 쓴다(생산 관리에서 한 번 걸렸다).

## 셸에 없어서 화면이 각자 만든 것 (올릴 후보)

| 없는 것 | 누가 아쉬웠나 | 지금 |
|---|---|---|
| 인페이지 탭 | 분석 서비스 | `.ptabs[data-style=line]` 마크업을 빌려 씀 |
| 제목 행 3단(제목+액션 / 상태 / 탭) | 프로젝트 | `#page-head[data-pj]` 로 덮어씀 — `mountShell({ subBelow, tabsRow })` 가 있으면 깔끔 |
| H1 파랑 룰 끄기 | 프로젝트 | 옵션 없음. 법전을 따르기로 하고 원판과의 차이를 기록 |
| 3열 분할 | 프로젝트 | `.split` 은 2열까지 — `pj-work` 를 따로 만듦 |
| 격자형 선택 카드 · 썸네일 체크 · 히스토그램 · 클래스 막대 · 지도 도구 열 | 분석 서비스 | `analysis.css` 안에 만듦 |
| `icon()` 의 `stack` · `refresh` | 프로젝트 | `layers` 로 대체 |

두 그룹 이상이 같은 것을 만들면 `parts.css` 로 올린다(성장 규칙 R2 와 같은 원칙).
